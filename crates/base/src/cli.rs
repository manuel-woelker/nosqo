...
pub fn try_main(run: impl FnOnce() -> NosqoResult<()>) -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
...
pub fn try_main_with_headline(headline: &str, run: impl FnOnce() -> NosqoResult<()>) -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
...
///
/// It returns a stable, human-readable rendering of a [`NosqoError`] that is
/// suitable for printing from a command-line binary.
pub fn format_cli_error(headline: &str, error: &NosqoError) -> String {
...
mod tests {
    ...
    fn format_cli_error_renders_headline_and_cause_chain() {
        let error = NosqoError::message("failed to verify")
            .with_source(NosqoError::message("missing reference output"));
